...
- `partial`, `uncertain`, `stale`, and `busy` stop the flow. The UI shows confirmed IDs/count and asks the user to reconcile active and archived lists. A lost response also stops without retry.
- The Archived view loads `getArchivedTasks` on request. Each archived task can be restored individually after confirmation through `unarchiveTask`. The server verifies expected archived state and readback; the result is shown before another action.
