...
Server functions provide snapshot and refresh operations, archived task metadata for unarchive review, individual archive and unarchive, and a group archive operation for one Automation ID. Each write checks fresh local state first and reads the result back. A group call handles at most ten runs. The caller must review the returned snapshot before continuing; `partial`, `uncertain`, `stale`, `busy`, or lost responses require manual reconciliation before another write. The dashboard reads snapshots and cached analysis, and starts or cancels Jev analysis only on explicit user action. It groups active automation runs by validated Automation ID and offers reviewed archive and restore controls.
